...
# TUSDC_DENOM is the denom of the tUSDC asset from Ethereum bridge.
TUSDC_DENOM="tusdc"
...
ETH_BRIDGE_ADDRESS="0xa4A7Acf2f06b1CC296E15E8979B546D34446D5c4"
TOTAL_NATIVE_TOKEN_SUPPLY=1000000000$EIGHTEEN_ZEROS # 1e27
BRIDGE_GENESIS_ACKNOWLEDGED_NEXT_ID=2
BRIDGE_GENESIS_ACKNOWLEDGED_ETH_BLOCK_HEIGHT=9541090

# Use a fix genesis time from the past.
GENESIS_TIME="2025-10-31T00:00:00Z"
...
function update_genesis_exchange() {
	GENESIS=$1/genesis.json
	btc_exchange_config_json=$(cat <<-EOF
	{
		"exchanges": [
	...
	POL_exchange_config_json=$(cat <<-EOF
	{
		"exchanges": [
			{
				"exchangeName": "Binance",
	...
	CRV_exchange_config_json=$(cat <<-EOF
	{
		"exchanges": [
			{
				"exchangeName": "Binance",
	...
	sol_exchange_config_json=$(cat <<-EOF
	{
		"exchanges": [
			{
				"exchangeName": "Binance",
	...
	atom_exchange_config_json=$(cat <<-EOF
	{
		"exchanges": [
			{
				"exchangeName": "Binance",
	...
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[6].exchange_config_json' -v "$atom_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[6].min_exchanges' -v '1'

	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[7].exchange_config_json' -v "$atom_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[7].min_exchanges' -v '1'

	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[8].exchange_config_json' -v "$atom_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[8].min_exchanges' -v '1'

	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[9].exchange_config_json' -v "$atom_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[9].min_exchanges' -v '1'

	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[10].exchange_config_json' -v "$atom_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[10].min_exchanges' -v '1'

   usdt_exchange_config_json=$(cat <<-EOF
       {
            "exchanges": [
                {
                    "exchangeName": "Binance",
	...
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[11].exchange_config_json' -v "$usdt_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[11].min_exchanges' -v '1'
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[12].exchange_config_json' -v "$usdt_exchange_config_json"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[12].min_exchanges' -v '1'
}

function edit_genesis_new() {
	GENESIS=$1/genesis.json
	...
	# Market: USDT-USD
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_params.[]' -v "{}"
	dasel put -t string -f "$GENESIS" '.app_state.prices.market_params.[12].pair' -v 'USDT-USD'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[12].id' -v '1000000'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[12].exponent' -v '-9'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_params.[12].min_price_change_ppm' -v '1000'  # 0.100%
	dasel put -t json -f "$GENESIS" '.app_state.prices.market_prices.[]' -v "{}"
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[12].id' -v '1000000'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[12].exponent' -v '-9'
	dasel put -t int -f "$GENESIS" '.app_state.prices.market_prices.[12].price' -v '1000000000'          # $1 = 1 USDT.
